...
private:
    int id;
    std::string name;
    std::string jobType;     // e.g. "Detailer", "Pressure Washer"
    double hourlyRate;
    double hoursWorked;
    std::string phone;
    std::string email;
...
public:
    ...
    static Employee* searchByName(std::vector<Employee>& employees, 
            std::string name);
    static std::vector<Employee> searchByJobType(std::vector<Employee>& employees,
            std::string jobType);
};
